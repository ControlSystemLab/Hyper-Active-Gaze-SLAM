...
def generate_launch_description():
    ...
    livox_gaze_control_dir = get_package_share_directory('livox_gaze_control')
    ...
    declare_visualization_config_file_cmd = DeclareLaunchArgument(
        ...
        default_value=os.path.join(
            livox_gaze_control_dir, 'config', 'grid.yaml'),
        description='Full path to the Gridmap visualization config file to use')

    declare_rviz_config_file_cmd = DeclareLaunchArgument(
        'rviz_config',
        default_value=os.path.join(
            livox_gaze_control_dir, 'rviz', 'grid_color.rviz'),
        description='Full path to the RVIZ config file to use')

    # Declare node actions
    simple_demo_node = Node(
        package='livox_gaze_control',
        executable='livox_gaze_control_node',
        name='livox_gaze_control_node',
        output='screen'
    )
